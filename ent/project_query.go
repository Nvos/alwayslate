...
func (pq *ProjectQuery) sqlAll(ctx context.Context) ([]*Project, error) {
	...
	if query := pq.withActivities; query != nil {
		...
		query.Where(predicate.Activity(func(s *sql.Selector) {
			s.Where(sql.InValues(project.ActivitiesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.project_activities
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "project_activities" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_activities" returned %v for node %v`, *fk, n.ID)
			}
