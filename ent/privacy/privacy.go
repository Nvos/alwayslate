...
// The TimesheetQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type TimesheetQueryRuleFunc func(context.Context, *ent.TimesheetQuery) error

// EvalQuery return f(ctx, q).
func (f TimesheetQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.TimesheetQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.TimesheetQuery", q)
}

// The TimesheetMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type TimesheetMutationRuleFunc func(context.Context, *ent.TimesheetMutation) error

// EvalMutation calls f(ctx, m).
func (f TimesheetMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.TimesheetMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.TimesheetMutation", m)
}
