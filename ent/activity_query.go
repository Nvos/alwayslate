...
// QueryTimesheets chains the current query on the "timesheets" edge.
func (aq *ActivityQuery) QueryTimesheets() *TimesheetQuery {
	query := &TimesheetQuery{config: aq.config}
...
func (aq *ActivityQuery) Clone() *ActivityQuery {
	...
	return &ActivityQuery{
		...
		withTimesheets: aq.withTimesheets.Clone(),
...
// WithTimesheets tells the query-builder to eager-load the nodes that are connected to
// the "timesheets" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ActivityQuery) WithTimesheets(opts ...func(*TimesheetQuery)) *ActivityQuery {
	query := &TimesheetQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withTimesheets = query
	return aq
}
...
func (aq *ActivityQuery) sqlAll(ctx context.Context) ([]*Activity, error) {
	...
	if query := aq.withTimesheets; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Activity)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Timesheets = []*Timesheet{}
		}
		query.withFKs = true
		query.Where(predicate.Timesheet(func(s *sql.Selector) {
			s.Where(sql.InValues(activity.TimesheetsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.activity_timesheets
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "activity_timesheets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "activity_timesheets" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Timesheets = append(node.Edges.Timesheets, n)
		}
	}
	...
	if query := aq.withProject; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_activities" returned %v`, n.ID)
			}
			for i := range nodes {
